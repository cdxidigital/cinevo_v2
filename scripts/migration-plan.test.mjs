...
test("the auth schema stays outside the globbed directory", () => {
  ...
  const rootEntries = readdirSync(migrationsDir);
  assert.ok(rootEntries.includes("0002_cinevo_user_state.sql"));
  assert.ok(rootEntries.includes("0003_shared_libraries.sql"));
  assert.ok(rootEntries.includes("0004_playback_library_scope.sql"));
  assert.equal(rootEntries.includes(AUTH_MIGRATION), false);
